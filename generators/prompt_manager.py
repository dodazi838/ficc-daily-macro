...
5. **daily_event_watchpoints (Daily Event 본문, 1문단 약 250~350자)**:
   ...
   - **A. 당일 주요 발표 리뷰 (무슨 결과가 나왔고 왜 중요한가 중심 복기)**:
     ...
     • **[수치 활용 및 정합성 원칙]**:
       - 경제지표의 실제치(actual), 시장 예상치(forecast), 전월치(prior)처럼 이벤트 자체를 이해하는 데 필요한 숫자는 적극적으로 활용한다 (표의 가격 등락률은 반복하지 않되 지표 수치는 적극 인용).
       - 실제치와 예상치가 SSOT에 모두 존재하면 실제치와 예상치의 상회/하회 여부 및 전월 대비 변화를 명확히 표현한다.
       - 실제치만 확인 가능한 경우에는 실제치와 그 의미만 설명한다.
       - 실제치가 SSOT에 부재하고 예상치/전월치만 확인되는 경우에는 확인 가능한 예상치/전월치 수준과 그 의미를 설명하며, 없는 숫자를 절대 임의로 날조하지 않는다.
     • **[주요 이벤트별 구체적 서술 기준]**:
       - **신규 실업수당 청구건수**: 예상치(205K) 및 전월치(203K) 수준과 노동시장 둔화/안정성 판단 재료로서의 의미를 명시.
       - **미국 ISM 서비스업 PMI**: 지수 수준(예상 54.2, 전월 54.1 등)의 50선 상회 흐름과 전월/예상 대비 변화를 통해 서비스업 경기 확장 모멘텀 및 경기 연착륙 평가를 명시.
       - **중앙은행 발언 (크리스토퍼 월러 연준 이사 등)**: 물가 둔화 지속 시 9월 금리 동결 선호 발언 등 확인된 정책 기조와 시장에 미친 영향 서술.
   - **B. 실행 시각 이후 주목할 이벤트 (향후 핵심 일정 안내)**:
     ...
     • 실행 시각 이후 예정된 지표의 시장 예상치(forecast) 숫자는 본문에 나열하지 않고 하단 Daily Event 표에서 제공한다.
   - **[이슈 리뷰와의 역할 분리]**:
     • Daily Event: 개별 지표의 발표 결과와 경제적 의미를 빠르게 복기하고 이후 일정을 안내.
     • 이슈 리뷰: 개별 이벤트를 단순 반복하지 않고, 여러 이벤트가 증시·외환·채권·원자재 4대 자산시장에 어떻게 종합적으로 연결·파급되었는지를 심층 분석.
