...
--------------------------------------------------
- `daily_event_watchpoints` 본문(1문단, 약 150~200자)에서 다루는 모든 경제지표는 **반드시 [Context JSON]의 `economic_calendar.today_night` 목록에 실제로 존재하는 canonical 지표여야 한다.**
- `today_night` 목록에 없는 지표(예: 16:30 이전 또는 전일 발표된 ISM 제조업 PMI, JOLTS 구인건수, 비농업 고용지수 등 과거 지표나 미예정 지표)를 절대 언급하거나 임의로 지어내지 마라.
- **[본문-표 역할 분리 및 예상치 병기 제거]**:
  • 본문: [발표 시각] + [이벤트명] + [시장에서 확인할 핵심 관전 포인트/중요성] 중심으로 서술한다.
  • 시장 예상치(forecast, 예: "예상 47K", "(예상 2.25%)", "0.7% 전망" 등)는 **본문에서 숫자로 병기하지 않는다.** (시장 예상치는 하단 Daily Event 표에만 표시).
- **[시각 표기 규칙]**:
  • 시각 뒤에 `KST` 영문 단어를 붙이지 말고 `01:15`, `02:45`, `03:00`과 같이 시간 숫자(HH:MM)만 간결하게 표기한다 (`01:15 KST` (X) ➡️ `01:15` (O)).
- 만약 `today_night` 목록이 비어있는 경우 "금일 16:30 이후 주요 발표 예정 지표 없음"으로 간결하게 작성하라.
