...
- 시장예상치(forecast)는 컨텍스트의 `forecast` 값만을 정확하게 인용하라.
  • 컨텍스트의 `forecast`가 null이거나 "-"인 지표는 예상치 숫자를 절대 지어내지 말고, 지표명과 발표 일정/관전 포인트만 서술하라.
  • "예상 55.2", "예상 7.33M" 등 원천 데이터에 없는 수치를 임의 추론/날조하여 작성하면 즉시 FAIL 처리된다.
- 발표 시각은 컨텍스트의 `scheduled_time_kst` (한국시간) 값을 정확히 인용하라.
