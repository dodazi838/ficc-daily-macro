...
5. **daily_event_watchpoints (Daily Event 본문, 1문단 약 250~300자)**:
   - 본문 구조는 **[당일 주요 발표 리뷰] → [실행 시각 이후 주목할 이벤트]** 2단계로 전개한다.
   - **A. 당일 주요 발표 리뷰**:
     • `economic_calendar.day_review` 중 시장 영향력이 큰 2~4개 이벤트(예: 신규 실업수당 청구건수, 미국 ISM 서비스업 PMI, 주요국 서비스업 PMI, CPI/PPI, 중앙은행 결정/인사 발언 등)를 선별.
     • 실제 발표 결과와 시장에 중요한 이유를 간결하게 복기 (forecast가 있으면 실제치와 비교 가능하나 forecast 수치를 본문에서 반복 나열하지 않음).
   - **B. 실행 시각 이후 주목할 이벤트**:
     • `economic_calendar.today_night`에 등록된 실행 시각 이후 예정된 핵심 지표(3~6개)의 [발표 시각(HH:MM)] + [이벤트명] + [시장에서 확인할 핵심 포인트]를 간결히 안내.
